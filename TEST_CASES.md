...
| **TC-AUTH-002** | Google Sign-In | Reject non-UMak domain emails | User not logged in | 1. Click "Sign in with Google"<br>2. Select a regular `@gmail.com` email | The server rejects access and the system displays an institutional-email notice. |
| **TC-AUTH-003** | Admin Email Bypass | Allow whitelisted admin emails | Email in the private database allowlist | 1. Sign in with a whitelisted non-UMak email | The server provisions an approved administrator profile. |
