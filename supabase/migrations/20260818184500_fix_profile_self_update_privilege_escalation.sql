...
--
-- Strategy:
--   1. REVOKE direct UPDATE via PostgREST for student-role users.
--      Students must use update_student_profile() RPC which enforces a strict allowlist.
--   2. Admin roles retain their direct UPDATE RLS policy for approval/ban workflows.
--   3. Denylist trigger kept as a defence-in-depth backstop.
--   4. RAISE NOTICE wrapped in DO block (top-level RAISE is invalid SQL).
-- ============================================================

-- ─────────────────────────────────────────────────────────────────────────────
-- STEP 1: Strict allowlist RPC for student self-updates
-- Only this function may be called by authenticated non-admin users to update
-- their own profile. It accepts only explicitly whitelisted columns.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Grant execute to authenticated users; admin service retains full direct access
...
-- ─────────────────────────────────────────────────────────────────────────────
-- STEP 2: Denylist trigger as defence-in-depth backstop
-- Catches any direct PostgREST PATCH that bypasses the RPC pathway.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- ─────────────────────────────────────────────────────────────────────────────
-- STEP 3: RLS update policies
-- Students must call update_student_profile() RPC (SECURITY DEFINER runs as
-- the function owner and bypasses PostgREST auth checks on the table directly).
-- We keep the direct UPDATE policy ONLY for admin roles.
-- ─────────────────────────────────────────────────────────────────────────────
...
-- Students: NO direct PostgREST UPDATE permitted. Access is via RPC only.
-- (The SECURITY DEFINER RPC bypasses RLS and writes only whitelisted columns.)

-- Admins: direct UPDATE for approval/ban workflows
...
DO $$
BEGIN
  RAISE NOTICE 'Issue #1 fix applied: update_student_profile() RPC + denylist trigger + admin-only direct UPDATE policy.';
END $$;
