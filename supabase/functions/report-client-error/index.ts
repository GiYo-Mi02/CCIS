...
} from "../_shared/supabase-keys.js";

const MAX_BODY_BYTES = 12 * 1024;
const MAX_STACK_TRACE_BYTES = 8192;
const UUID_RE =
...
serve(async (req: Request) => {
  ...
  const publishableKey = resolveSupabasePublishableKey((name) =>
    Deno.env.get(name)
  );
  ...
  if (rateError) {
    return json(500, { error: "RATE_LIMIT_CHECK_FAILED" }, corsHeaders);
  }
